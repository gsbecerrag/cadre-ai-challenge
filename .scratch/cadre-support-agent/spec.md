...
- A fifth seam since 2 Sep: `ResearchSource`, through which the Research Agent reads public information; its only implementation is the fixture adapter until a live web or company-data source lands in Phase 2. (ADR-0011)
...
- Pages: Availability (online toggle bound to the Strategist's presence document), Handover queue (pending, in call, callbacks), a request detail with Lead, signals, score, conversation, Join and End, and a Triage tab listing Triage Reports newest first with a link to the Trace. Since 2 Sep, every Qualified Lead card also carries the Research Agent's action and, once it has run, the Lead Brief (ADR-0011).
...
### Research Agent (scope addition, 2 Sep)

- A Strategist runs the Research Agent on one Qualified Lead from the Console; a Session with no Lead and a Lead below the threshold are refused. The run happens inside the Console request and streams Server-Sent Events: a progress line per step, each Research Finding as it lands, the finished Lead Brief, a final event with the Trace id and usage, and an error event with a safe message. (ADR-0011)
- The agent is a plain tool loop of the Turn's shape, at most four iterations, with three tools run in code: `search_web`, `lookup_company` and `write_brief`. The brief is the arguments of `write_brief`, validated where the tool runs; a run that reaches the cap without one gets a brief assembled from the Findings so far, marked incomplete. The prompt is the Assistant's cached prefix byte for byte with the research instructions in the volatile tail, so the talking points are grounded in Cadre's services and the call lands on the warm cache.
- A Lead Brief carries a company snapshot, a person snapshot, talking points, one advisory note per Qualification Signal, the Research Findings consulted, the Research Source's name, the model, the cost, when it was written and whether it completed. It is keyed by the Session like the Lead; running again overwrites. Research never changes the Qualification Score (ADR-0009).
- The `ResearchSource` seam has one implementation today: canned Findings for a few demo companies keyed by company name or email domain, and a deterministic "no public record found" answer for every other Lead. The deployed demo runs the real model over these fixtures and says so on the brief.
- The Console shows "Research this Lead" on every Qualified Lead card, the progress and Findings while a run is in flight, and the brief afterwards; briefs also arrive through a realtime listener with the same polling fallback as the other feeds.
...
- **S3 — independent agent handlers.** The Triage Agent's handler called with a fake Firestore event and a fake client and the stub provider: thumbs-up is a no-op; thumbs-down writes a Triage Report with the schema; a redelivered event does not write twice. The Research Agent's handler called with a Qualified Lead in the in-memory store, the fixture Research Source, the stub provider scripted to call the tools, and the recording tracer: it yields progress, Findings and the brief in order, persists the brief, and assembles a cut-short brief at the iteration cap.
- **S4 — web.** One Vitest test for the chat reducer: the SSE event sequence produces the expected chat state (streamed text, card, offer, hand-over states, error). One more for the Console's research reducer: progress, Finding, brief and error produce the expected card state.
...
- A live Research Source (web search or a company-data API), the automatic research run when a Lead becomes Qualified, research on a Lead that is not Qualified, research feeding the Qualification Score, and a conversational research panel (Phase 2, ADR-0011).
