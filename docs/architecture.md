...
flowchart TB
  ...
  subgraph gcp["GCP project"]
    ...
    fs[("Firestore Native, nam5: sessions, messages, leads, handover_requests, strategists, feedback, triage_reports, lead_briefs")]
...
The chat API never learns that the triage agent exists. Firestore is the event bus, and the same trigger pattern hosts future agents (the Research Agent's automatic run on qualification, hand-over timeout) without touching the request path. Today the Research Agent runs on demand inside the Console request instead — a Strategist is waiting and wants to watch — through the same seam-pure handler shape, so the trigger is the only thing that changes later (ADR-0011).
...
erDiagram
  ...
  LEADS {
    string id PK "the session id: one lead per session"
    string session_id FK
    map signals "BANT-lite inputs from capture_lead"
    int score "0 to 5, computed in code"
    bool qualified "score at or above QUALIFICATION_THRESHOLD"
  }
  HANDOVER_REQUESTS {
    string id PK
    string session_id FK
...
| `pending_strategist` | Strategist claims the request in the console | `strategist_joined` | `strategist_name` set; validated and written together with the hop below, because "Claim & join call" is one button |
| `pending_strategist` | nobody claims within `HANDOVER_JOIN_TIMEOUT_SECONDS` | `no_strategist_available` | Terminal; `mode` flips to `callback` and the lead is already captured. Answered lazily on the widget's own status poll (`GET /api/handover/{id}`), not by a scheduler: no second process, no second clock, and nothing left running for a visitor who closed the tab |
...
flowchart TB
  ...
  subgraph gcp["GCP project"]
    ...
    subgraph fns["Firebase Functions, Firestore-triggered agents"]
      ...
      enrich["Research Agent, automatic on qualification (Phase 2; on demand today)"]
